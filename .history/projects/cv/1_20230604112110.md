...
As seen from the confusion matrix, the sparse matrix classifies the drawings with high accuracy. However for certain classes such as M7-M13, there is a misclassification. Upon checking the drawings, we can see that the drawings are somehow similar such that the difference are at the fingertip or the face level. This opens up for further research on how to effectively capture these minute details.
